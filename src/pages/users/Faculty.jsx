...
const SAMPLE_FACULTY = [
  { id: 2, name: 'Sara Johnson', email: 'sara.j@zinggerr.com', username: 'sara_j', phone: '+1 555 234 5678', role: 'Faculty', gender: 'Female', status: true, avatar: '' },
  { id: 4, name: 'Emily Chen', email: 'emily.c@zinggerr.com', username: 'emily_c', phone: '+1 555 876 5432', role: 'Faculty', gender: 'Female', status: true, avatar: '' },
  { id: 8, name: 'Neha Gupta', email: 'neha.g@zinggerr.com', username: 'neha_g', phone: '+91 88776 65544', role: 'Faculty', gender: 'Female', status: true, avatar: '' },
];
    ...
    <Backdrop onClose={onClose}>
      <div className="max-w-md mx-auto rounded-2xl p-6 glass-surface shadow-glass">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-bold text-main">Faculty Details</h3>
  ...
  return (
    <Backdrop onClose={onClose}>
      <div className="max-w-sm mx-auto rounded-2xl p-6 glass-surface shadow-glass text-center">
        ...
        <h3 className="text-lg font-bold text-main mb-2">Delete Faculty</h3>
  ...
  return (
    <Backdrop onClose={onClose}>
      <div className="max-w-lg mx-auto rounded-2xl p-6 glass-surface shadow-glass max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-bold text-main">Edit Faculty</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[var(--border)] transition-colors">
            <X size={18} className="text-muted-custom" />
          </button>
        </div>
...
// ── Main Component ──
const FacultyList = () => {
  const [faculty, setFaculty] = useState(SAMPLE_FACULTY);
  ...
  const uniqueRoles = useMemo(() => ['All', ...new Set(faculty.map(u => u.role))], [faculty]);
  ...
  const toggleStatus = (id) => setFaculty(prev => prev.map(u => u.id === id ? { ...u, status: !u.status } : u));
  const handleDelete = (id) => setFaculty(prev => prev.filter(u => u.id !== id));
  const handleSave = (updated) => setFaculty(prev => prev.map(u => u.id === updated.id ? updated : u));
    ...
    <div className="space-y-6">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}
        ...
        <div>
          ...
          <p className="text-sm text-muted-custom mt-1">Manage all faculty members in your system</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['#', 'Faculty', 'Username', 'Phone', 'Role', 'Gender', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              ...
              {paginated.length === 0 && (
                <tr><td colSpan={8} className="px-5 py-12 text-center text-muted-custom">No faculty members found</td></tr>
              )}
            </tbody>
          </table>
        </div>
